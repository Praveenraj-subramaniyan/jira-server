...
async function InsertSignUpUser(token) {
  ...
  `;
      sendMail(
        newUser.email,
        "Jira Clone - Registration Successful",
        content
      );
      return `
      <html>
        ...
        <body>
          ...
          <p>Your account has been successfully registered in Jira clone.</p>
          <p>Click below link to login into your account.</p>
          <a href="https://stack-overflow-clone-praveenraj.netlify.app/">https://stack-overflow-clone-praveenraj.netlify.app/</a>
        </body>
      </html>`;
...
async function InsertVerifyUser(name, email, password) {
  try {
    ...
    await newUser.save();
    sendMail(email, "Jira Clone - Verify User", content);
  } catch (error) {
    console.error(error);
  }
}
